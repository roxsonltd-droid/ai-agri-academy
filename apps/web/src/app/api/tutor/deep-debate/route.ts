...
/** Proxies to FastAPI `POST /api/tutor/deep-debate` (multi-agent LangGraph debate). */
export async function POST(req: NextRequest) {
	try {
		...
		const useDebate = body.useDebate !== false;
		...
		const response = await fetch(`${backendUrl}/api/tutor/deep-debate`, {
			method: "POST",
		...
		console.error("POST /api/tutor/deep-debate proxy:", error);
