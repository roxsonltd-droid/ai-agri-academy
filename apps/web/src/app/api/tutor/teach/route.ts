...
/** Proxies to FastAPI `POST /api/tutor/teach` (adaptive / static / A/B). */
export async function POST(req: NextRequest) {
	try {
		const body = await req.json();
		const backendUrl = serverBackendBaseUrl();
		const response = await fetch(`${backendUrl}/api/tutor/teach`, {
			method: "POST",
		...
		console.error("POST /api/tutor/teach proxy:", error);
